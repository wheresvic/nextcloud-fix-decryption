...
class Crypt
{
    ...
    public function __construct($logger, $user, $instanceId, $instanceSecret)
    {
        $this->logger = $logger;
        $this->user = $user;
    ...
    {
        $salt = hash('sha256', $uid . $this->instanceId . $this->instanceSecret, true);
    ...
    private function decrypt($encryptedContent, $iv, $passPhrase = '', $cipher = self::DEFAULT_CIPHER)
    {
        ...
        if ($plainContent) {
            ...
            throw new \Exception('Encryption library: Decryption (symmetric) of content failed: ' . openssl_error_string());
        }
    }
    ...
    {
        if (!$encKeyFile) {
            throw new \Exception('Cannot multikey decrypt empty plain content');
        }

        if (openssl_open($encKeyFile, $plainContent, $shareKey, $privateKey)) {
            return $plainContent;
        } else {
            throw new \Exception('multikeydecrypt with share key failed:' . openssl_error_string());
        }
    }
    ...
    public function multiKeyEncrypt($plainContent, array $keyFiles)
    {
        ...
        if (empty($plainContent)) {
            throw new \Exception('Cannot multikeyencrypt empty plain content');
        }
        ...
        if (openssl_seal($plainContent, $sealed, $shareKeys, $keyFiles)) {
            ...
            throw new \Exception('multikeyencryption failed ' . openssl_error_string());
        }
    }
}
